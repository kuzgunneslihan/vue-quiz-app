...
const createRequire = globalThis.process?.getBuiltinModule("node:module")?.createRequire;
/**
...
function getRequireFromLinter() {
	...
	try {
		...
		const relativeTo = path.join(path.dirname(eslintPkgPath), "__placeholder__.js");
...
function loadNewest(items) {
	...
	for (const item of items) {
		...
		if (pkg != null && (!target || isLessOrEqual(target.version, pkg.version))) target = {
			version: pkg.version,
			get: item.get
		};
	}
