...
export function createDocumentationService() {
    return {
        getInstallationGuide: async ({ fresh, platform, ssr }) => {
            ...
            let text = INSTALLATION_PLATFORMS[platform].markdown;
            if (fresh) {
                const freshText = FRESH_INSTALLATION_PLATFORMS[platform];
        ...
        getV4BreakingChanges: async ({ category }) => {
            ...
            const text = `# Vuetify 4 Breaking Changes\n\n`
                ...
                + Object.entries(V4_BREAKING_CHANGES).map(([key, cat]) => (`## ${cat.name} (${key})\n\n${cat.description}\n\n`
        ...
        getReleaseNotesByVersion: async ({ version }) => {
