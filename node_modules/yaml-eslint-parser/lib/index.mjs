...
const resolver = {
	...
	YAMLMapping(node, version) {
		...
		for (const pair of node.pairs) {
			const mergeSources = version === "1.1" && isMergeKeyPair(pair) ? toMergeSources(getValue(pair.value, version)) : null;
			if (mergeSources) {
				for (const source of mergeSources) for (const key of Object.keys(source)) if (!Object.prototype.hasOwnProperty.call(result, key)) result[key] = source[key];
			} else Object.assign(result, getValue(pair, version));
		}
		return result;
	},
	...
	YAMLSequence(node, version) {
		const result = [];
		for (const entry of node.entries) result.push(entry ? getValue(entry, version) : null);
		return result;
	},
	YAMLScalar(node) {
		return node.value;
	...
	YAMLWithMeta(node, version) {
		if (node.tag) {
			...
			if (value == null) return getTaggedValue(node.tag, "", "", version);
			if (value.type === "YAMLScalar") {
				if (value.style === "plain") return getTaggedValue(node.tag, value.strValue, value.strValue, version);
				if (value.style === "double-quoted" || value.style === "single-quoted") return getTaggedValue(node.tag, value.raw, value.strValue, version);
			}
			for (const tagResolver of tagNodeResolvers[version || "1.2"]) if (tagResolver.tag === node.tag.tag && tagResolver.testNode(value)) return tagResolver.resolveNode(value);
		}
...
*/
function isMergeKeyPair(pair) {
	const key = pair.key;
	return pair.value != null && key != null && key.type === "YAMLScalar" && key.style === "plain" && key.strValue === MERGE_KEY;
}
/**
...
*/
function toMergeSources(value) {
	if (Array.isArray(value)) {
		const sources = [];
		for (const item of value) {
			if (!isMapValue(item)) return null;
			sources.push(item);
		}
		return sources;
	}
...
function getTaggedValue(tag, text, str, version) {
	for (const tagResolver of tagResolvers[version || "1.2"]) if (tagResolver.tag === tag.tag && tagResolver.testString(str)) return tagResolver.resolveString(str);
	const tagText = tag.tag.startsWith("!") ? tag.tag : `!<${tag.tag}>`;
	return parseDocument(`${version ? `%YAML ${version}` : ""}
---
${tagText} ${text}`).toJSON();
}
//#endregion
...
function convertPlain(preTokens, cst, node, ctx, parent, doc) {
	...
	*/
	function parseValueFromText(str, version) {
		for (const tagResolver of tagResolvers[version]) if (tagResolver.testString(str)) return tagResolver.resolveString(str);
		return str;
	}
}
/**
...
function convertAnchor(cst, ctx, parent, doc) {
	...
	(doc.anchors[name] || (doc.anchors[name] = [])).push(ast);
