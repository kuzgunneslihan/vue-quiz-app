...
function readPnpmWorkspace(filepath) {
	...
	const queueChange = (fn, order) => {
		...
		queueTimer = setTimeout(() => {
			...
			const clone = [...queue];
			queue.length = 0;
			for (const fn of clone) fn(workspace);
...
function getWorkspacePackageJsonPaths(workspace) {
	...
	if (packages === void 0) return /* @__PURE__ */ new Set([rootPackageJson, ...globPackageJsonPaths(root, ["**/package.json"], PACKAGE_JSON_GLOB_IGNORE)]);
	if (!Array.isArray(packages)) return /* @__PURE__ */ new Set([rootPackageJson]);
	const packageJsonGlobs = getPackageJsonPatterns(packages);
	if (!packageJsonGlobs) return /* @__PURE__ */ new Set([rootPackageJson]);
	return /* @__PURE__ */ new Set([rootPackageJson, ...globPackageJsonPaths(root, packageJsonGlobs.include, packageJsonGlobs.exclude)]);
}
function getPackageJsonPatterns(packages) {
	const include = [];
	const exclude = [...PACKAGE_JSON_GLOB_IGNORE];
	for (const pattern of packages) {
		if (typeof pattern !== "string") return void 0;
		if (pattern.startsWith("!")) exclude.push(toPackageJsonGlob(pattern.slice(1)));
		else include.push(toPackageJsonGlob(pattern));
	}
	return {
		include,
		exclude
...
function globPackageJsonPaths(root, includeGlobs, excludeGlobs) {
	...
	for (const path of packageJsonPaths) paths.add(normalize(path));
	packageJsonPathCache.set(cacheKey, paths);
	return paths;
}
function isWorkspacePackageJson(filename, workspace) {
	if (!filename.endsWith("package.json")) return false;
	const target = normalize(resolve(resolve(dirname(workspace.filepath)), filename));
	return getWorkspacePackageJsonPaths(workspace).has(target);
}
function toPackageJsonGlob(pattern) {
	return pattern.endsWith("package.json") ? pattern : join(pattern, "package.json");
}
//#endregion
//#region src/rules/json/json-enforce-catalog.ts
const RULE_NAME$7 = "json-enforce-catalog";
...
var json_enforce_catalog_default = createEslintRule({
	...
	create(context, [options]) {
		...
		if (!workspace || !isWorkspacePackageJson(context.filename, workspace)) return {};
...
var json_prefer_workspace_settings_default = createEslintRule({
	name: "json-prefer-workspace-settings",
	meta: {
		type: "layout",
...
const RULE_NAME$5 = "json-valid-catalog";
...
const rules$2 = {
	...
	"json-valid-catalog": createEslintRule({
		name: RULE_NAME$5,
		meta: {
			type: "layout",
...
var yaml_no_anonymous_catalog_default = createEslintRule({
	name: "yaml-no-anonymous-catalog",
	meta: {
		type: "problem",
		docs: { description: "Disallow the anonymous `catalog:` in `pnpm-workspace.yaml` in favor of named catalogs" },
		schema: [],
		messages: { unexpectedAnonymousCatalog: "Avoid the anonymous `catalog:`. Use a named catalog (`catalogs: <name>:`) instead." }
	},
	defaultOptions: [],
	create(context) {
		if (basename(context.filename) !== "pnpm-workspace.yaml") return {};
...
var yaml_no_duplicate_catalog_item_default = createEslintRule({
	name: "yaml-no-duplicate-catalog-item",
	meta: {
		type: "problem",
	...
	create(context, [options = {}]) {
		...
		for (const [catalog, object] of Object.entries(catalogs)) {
			...
			for (const [key, version] of Object.entries(object)) {
				...
				if (existing) {
					if (checkDuplicates === "name-only" ? true : existing.version === version) {
...
var yaml_no_unused_catalog_item_default = createEslintRule({
	name: "yaml-no-unused-catalog-item",
	meta: {
		type: "problem",
	...
	create(context) {
		...
		const packages = Array.from(getWorkspacePackageJsonPaths(workspace)).filter((x) => existsSync(x)).sort();
		const FIELDS = [
			"dependencies",
			"devDependencies",
			"peerDependencies",
		...
		for (const path of packages) {
			...
			for (const field of FIELDS) {
				...
				for (const [name, value] of Object.entries(map)) {
					...
					const key = `${name}:${value.slice(8) || "default"}`;
					entries.delete(key);
				}
			}
		}
...
const rules$1 = {
	...
	"yaml-no-anonymous-catalog": yaml_no_anonymous_catalog_default,
	"yaml-valid-packages": createEslintRule({
		name: RULE_NAME,
		meta: {
			type: "problem",
