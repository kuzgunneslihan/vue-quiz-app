...
var indent_default = createRule("indent", {
	...
	create(context) {
		...
		*/
		function createErrorMessageData(expectedAmount, actualIndent) {
			const expectedStatement = `${expectedAmount} ${indentType}${expectedAmount === 1 ? "" : "s"}`;
			const groups = actualIndent.join("").match(/ +|\t+/gu) ?? [];
			let foundStatement = "";
			if (groups.length === 0) foundStatement = "0";
			else foundStatement = groups.map((group) => {
				const count = group.length;
				const groupIndentType = group.startsWith("	") ? "tab" : "space";
				if (groups.length === 1 && groupIndentType === indentType) return String(count);
				return `${count} ${`${groupIndentType}${count === 1 ? "" : "s"}`}`;
			}).join(" ");
			return {
...
*/
function parseNestedOrder(order) {
	if (order?.type === "ignore") return {
		ignore: true,
		isValidNestOrder: () => true
	};
	return {
		ignore: false,
		isValidNestOrder: buildValidatorFromType(order?.type ?? "asc", order?.caseSensitive === false, Boolean(order?.natural))
	};
}
/**
* Parse options
...
const IGNORE_ORDER_OBJECT_SCHEMA = {
	type: "object",
					...
					order: { oneOf: [
						{
							...
							items: { anyOf: [{ type: "string" }, {
								...
								properties: {
									...
									order: { oneOf: [ORDER_OBJECT_SCHEMA, IGNORE_ORDER_OBJECT_SCHEMA] }
								},
								additionalProperties: false
							}] },
							uniqueItems: true
		...
		*/
		function groupingProperties(properties, option) {
			...
			for (const property of properties) {
				...
				prev = property;
				if (option.ignore(property)) continue;
				group.push(property);
			}
